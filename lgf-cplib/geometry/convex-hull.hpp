...
// Casco convexo CCW, O(N log N). Colineares são descartados.
template<typename T>
...
// Struct para operações O(log n) em casco convexo (CCW, sem colineares).
...
struct ConvexPol {
    ...
    // ponto extremo: cmp(p, q) = true se p é mais extremo que q
    ...
    int max_dot(Ptd v) {
        return extreme([&](Ptd p, Ptd q) { return p.dot(v) > q.dot(v); });
    }

    // tangentes a partir de p (índices {esquerda, direita}) — O(log n)
