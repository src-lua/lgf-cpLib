 ...
 *   comp[v]  = id do SCC de v (0-indexado, ordem topológica reversa)
 ...
 *
 * Nota: comp[] está em ordem topológica reversa do DAG condensado,
 *       i.e., se há aresta SCC a → SCC b, então comp[a] > comp[b].
 *       Para ordem topológica direta, percorra de n_scc-1 a 0.
 */
...
struct Tarjan {
    ...
    Tarjan(const vector<vector<int>>& adj)
        : n(adj.size()), g(adj), comp(adj.size(), -1) { build(); }
