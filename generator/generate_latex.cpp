...
const bool ADD_DESC = false;  // Desativado inicialmente - ative quando quiser adicionar descricoes
const string DESC_BGN = "LATEX_DESC_BEGIN";
const string DESC_END = "LATEX_DESC_END";

const bool BLOCK_DESC = false;  // Desativado inicialmente - ative quando quiser adicionar block descriptions
...
bool convert_files(const string& input_path, const string& output_path, string& description, bool PRINT_HASH = false){
    ...
    while(pos < content.size()){
        ...
        if(isInDesc && !ignore){
            if(ADD_DESC) description += line + "\n";
            continue; // Always skip description lines
        }
