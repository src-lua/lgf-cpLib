 ...
 * O hash de uma linha com } é o hash de todo o codigo desde a { que a abriu.
 * Uso:
 *   g++ hash.cpp -o hash 
 *   ./hash < code.cpp
 */
...
/*LATEX_DESC_BEGIN
Callto get the hash of the code.
...
string getCodeHash(const string& code, int dig=3){
	...
	while(getline(ss, l)){
        ...
        // Verificar se a linha é apenas comentário ou vazia
        string trimmed_line = l;
        while(!trimmed_line.empty() && isspace(trimmed_line.front())) trimmed_line.erase(0, 1);
        while(!trimmed_line.empty() && isspace(trimmed_line.back())) trimmed_line.pop_back();

        bool is_comment_line = trimmed_line.empty() ||
                               (trimmed_line.size() >= 2 && trimmed_line.substr(0, 2) == "//") ||
                               (trimmed_line.size() >= 2 && trimmed_line.substr(0, 2) == "/*") ||
                               (trimmed_line.size() >= 1 && trimmed_line[0] == '*');
		if (is_comment_line && trimmed_line.size() >= 2 && trimmed_line.substr(0, 2) == "/*") block_comment = true;

		is_comment_line = is_comment_line || block_comment;

		if (block_comment && trimmed_line.size() >= 2 && trimmed_line.find("*/") != string::npos) block_comment = false;

        if(is_comment_line) {
            // Linha vazia ou apenas comentário -> hash vazio
            hs = "";
            for(int i = 0; i < dig; i++) hs += " ";
        } else {
            // Calcular hash do contexto (t) normalmente
