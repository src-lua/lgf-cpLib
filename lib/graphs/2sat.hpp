 ...
 *
 *   add_impl(x, y)  → x → y  (contrapositiva ~y → ~x automática)
 *   add_or(x, y)    → x ∨ y
...
struct TwoSat {
    ...
    // converte literal para nó interno: i >= 0 → 2*i, i < 0 → -2*i-1
    ...
    void add_xor(int x, int y) { add_or(x, y); add_or(~x, ~y); }

    // x ↔ y
    void add_iff(int x, int y) { add_impl(x, y); add_impl(~x, ~y); }
