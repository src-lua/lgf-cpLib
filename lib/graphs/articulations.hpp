 ...
 *
 * Um vértice é ponto de articulação se sua remoção desconecta o grafo.
 *
 * Campos públicos após build():
 *   is_ap[v] = true se v é ponto de articulação
 *   get()    = lista de vértices que são pontos de articulação
 */
...
struct Articulations {
    ...
    Articulations(const vector<vector<int>>& adj)
        : n(adj.size()), g(adj), is_ap(adj.size(), false) { build(); }
    ...
    // Retorna lista de vértices que são pontos de articulação
