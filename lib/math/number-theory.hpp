 ...
 * Retorna {0, 0} se não há solução (m1, m2 não coprimos e incompatíveis).
 */
...
pair<ll,ll> crt(ll a1, ll m1, ll a2, ll m2) {
    ...
    for (ll ta = a, tb = b; tb; ) {
        ll q = ta/tb; ta -= q*tb; swap(ta,tb); x -= q*y; swap(x,y);
    }
    x = (x % (m2/g) + m2/g) % (m2/g);
    ll r = (a1 + m1 * ((a2 - a1) / g % (m2/g) * x % (m2/g))) % l;
    return {(r + l) % l, l};
}

/* Fatoração usando lista de primos pré-computada — O(π(sqrt(N)))
 * Use quando já tiver rodado sieve(sqrt(n)) antes de chamar para vários n.
