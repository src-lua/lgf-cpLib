...
template <typename... A>
struct is_stack<stack<A...>> : true_type {};

template <typename T>
struct is_queue : false_type {};
template <typename... A>
struct is_queue<queue<A...>> : true_type {};

template <typename T>
struct is_pq : false_type {};
template <typename... A>
struct is_pq<priority_queue<A...>> : true_type {};
...
inline void print_128(__int128 x) {
  ...
  cerr << static_cast<char>('0' + static_cast<ll>(x % 10));
}
...
void debug_impl(const char *names, ll line,
                const Args &...args) {
  cerr << "\033[34m" << "[L" << line << "] "
       << "[" << names << "]"
       << "\033[0m" << " = "
...
#ifdef LOCAL
#define debug(...)                                              \
  lgf_dbg::debug_impl(#__VA_ARGS__, __LINE__, __VA_ARGS__)
#else
#define debug(...) ((void)0)
#endif
