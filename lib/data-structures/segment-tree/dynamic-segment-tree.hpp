...
struct DynamicSegTree {
  ...
  void update(int v, ll l, ll r, ll ql, ll qr, const TAG &t) {
    ...
    NODE lc = (seg[v].l != -1) ? seg[seg[v].l].node : NODE();
    NODE rc = (seg[v].r != -1) ? seg[seg[v].r].node : NODE();
    seg[v].node = NODE::merge(lc, rc);
  }
  ...
  void update(ll ql, ll qr, const TAG &t) {
    update(0, L, R, ql, qr, t);
  }
