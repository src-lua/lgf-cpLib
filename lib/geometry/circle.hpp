...
vector<Ptd> circ_line_inter(Ptd a, Ptd b, Ptd c, ld r) {
  ...
  points.push_back(c + v + u * (-B + sqrt(D + eps)) / A);
  if (D > eps) points.push_back(c + v + u * (-B - sqrt(D)) / A);
  return points;
}
...
vector<Ptd> circ_inter(Ptd a, Ptd b, ld r, ld R) {
  ...
  Ptd v = (b - a) / d;
  points.push_back(a + v * x + v.perp() * y);
  if (y > eps) points.push_back(a + v * x - v.perp() * y);
  return points;
}
