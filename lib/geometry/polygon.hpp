...
// O(n) para qualquer polígono simples, convexo ou côncavo.
template <typename T>
...
// Corta pela reta r; mantém p quando (r.p, r.q, p) é CCW.
vector<Ptd> cut_polygon(vector<Ptd> poly, Line r) {
  ...
  for (int i = 0; i < n; i++) {
    if (ccw(r.p, r.q, poly[i])) res.push_back(poly[i]);
    if (n == 1) continue;
    Line s(poly[i], poly[(i + 1) % n]);
    Ptd p = inter(r, s);
    if (isinseg(p, s)) res.push_back(p);
  }
  res.erase(unique(res.begin(), res.end()), res.end());
  if (res.size() > 1 && res.back() == res[0]) res.pop_back();
  return res;
}
...
bool interpol(const vector<Ptd> &v1, const vector<Ptd> &v2) {
  ...
  for (auto &p : v1)
    if (winding(v2, p)) return true;
  for (auto &p : v2)
    if (winding(v1, p)) return true;
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++)
...
ll lattice_points(ll area2, ll boundary) {
