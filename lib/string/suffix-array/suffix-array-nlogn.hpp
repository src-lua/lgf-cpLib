...
struct SuffixArray {
    ...
    void build() {
        ...
        rk[sa[0]] = 0;
        for (int i = 1; i < n; i++)
            rk[sa[i]] = rk[sa[i - 1]] + (a[i].first != a[i - 1].first ? 1 : 0);
        ...
        for (int k = 0; (1 << k) < n; k++) {
            ...
            new_rk[sa[0]] = 0;
            for (int i = 1; i < n; i++) {
                pair<int,int> prv = {rk[sa[i-1]], rk[(sa[i-1] + (1 << k)) % n]};
                pair<int,int> now = {rk[sa[i]],   rk[(sa[i]   + (1 << k)) % n]};
                new_rk[sa[i]] = new_rk[sa[i - 1]] + (now != prv ? 1 : 0);
            }
            rk = new_rk;
        }
    }
    ...
    // Retorna [lo, hi) no SA onde p ocorre. hi - lo = nº de ocorrências.
    pair<int,int> search(const string& p) const {
        int lo, hi, n = sa.size();
        {
            ...
            while (l < r) {
                int m = (l + r) / 2;
                if (s.compare(sa[m], p.size(), p) < 0) l = m + 1; else r = m;
            }
            lo = l;
        }
        {
    ...
    static string lcs(const string& a, const string& b, char sep = '$') {
        ...
        for (int i = 1; i < (int)suf.sa.size(); i++) {
            bool ga = suf.sa[i] < m, gb = suf.sa[i - 1] < m;
            if (ga != gb && suf.lcp[i] > best)
                { best = suf.lcp[i]; pos = suf.sa[i]; }
        }
