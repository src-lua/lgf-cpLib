 ...
 *
 * Métodos:
 *   search(p) → [lo, hi): intervalo no SA onde p ocorre; hi-lo = nº ocorrências
 */
...
struct SuffixArrayLinear {
    ...
    static vector<int> sa_is(const T& s, int sigma) {
        ...
        auto induced_sort = [&](const vector<int>& lms) {
            ...
            b = bkt;
            for (int i = n - 1; i >= 0; i--) if (sa[i] > 0 && t[sa[i] - 1])
        ...
        for (int i = 0, prev = -1; i < n; i++) {
            ...
            if (prev != -1) {
                bool diff = false;
                for (int d = 0; ; d++) {
                    if (s[prev+d] != s[sa[i]+d] || t[prev+d] != t[sa[i]+d]) {
                        diff = true; break; 
                    }
                    if (d > 0 && (is_lms(prev+d) || is_lms(sa[i]+d))) break;
                }
                if (diff) cls++;
            }
            rank_[sa[i]] = cls;
